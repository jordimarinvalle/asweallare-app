 ...
 * For any {collection_series_id, level}: if user has any active non-sample box,
 * hide the sample box for that level.
 * @param {string|null} userId - The user's ID
...
export async function getBoxesWithVisibility(userId) {
  ...
  // Determine which levels the user has non-sample access to
  const levelsWithFullAccess = new Map() // Map<`${series}_${level}`, boolean>
  
  boxes.forEach(box => {
    if (!box.is_sample && entitledBoxIds.includes(box.id)) {
      const key = `${box.collection_series_id}_${box.level}`
      levelsWithFullAccess.set(key, true)
    }
  })
  
  // Filter boxes based on visibility rules
  const visibleBoxes = boxes.filter(box => {
    // Non-sample boxes are always visible (access is separate from visibility)
    if (!box.is_sample) return true
    
    // Sample boxes: hide if user has non-sample access at same series+level
    const key = `${box.collection_series_id}_${box.level}`
    return !levelsWithFullAccess.has(key)
  })
