...
export async function POST(request) {
  ...
  try {
    // Delete all cards for a box/pile combination
    if (path === 'admin/cards/bulk-delete') {
      const { user, error: authError } = await getAuthenticatedUser()
      ...
      let query = supabase.from('cards').delete().eq('box_id', boxId)
      ...
      return handleCORS(NextResponse.json({ success: true, message: 'Cards deleted' }))
    }
    
    // Get cards for download (returns list of image paths)
    if (path === 'admin/cards/export') {
      const { user, error: authError } = await getAuthenticatedUser()
