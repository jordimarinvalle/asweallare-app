...
function ProfileScreen({
  ...
  const { theme, colorMode, autoColorMode, colorModeOverride, setColorModePreference, colors } = useTheme()
              ...
              <div>
                ...
                {showBoxForm && (
                  <Card className="p-6 mb-6">
                    ...
                    <div className="flex gap-2 mt-4">
                      <Button onClick={handleSaveBox} className="h-9 px-4 text-sm font-medium bg-red-600 hover:bg-red-700 text-white">{editingBox ? 'Update' : 'Create'}</Button>
                      <Button onClick={() => { setShowBoxForm(false); setEditingBox(null); setUploadResult(null); setBulkUploadPileId('') }} variant="outline" className="h-9 px-4 text-sm font-medium">Cancel</Button>
                    </div>
                      ...
                      <Card key={box.id} className="p-4">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex gap-1">
                            ...
                            <Button onClick={() => { if(confirm('Delete this box?')) handleDeleteBox(box.id) }} size="sm" variant="ghost" className="h-8 w-8 p-0 text-red-600 hover:text-red-700"><Trash2 className="w-4 h-4" /></Button>
                          </div>
                        </div>
                      </Card>
                    )
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div className="sm:col-span-2 lg:col-span-3">
                        ...
                        {pileForm.imagePath && (
                          <div className="mt-2">
                            <img 
                              src={getImageUrl(pileForm.imagePath)} 
                              alt="Pile preview" 
                      ...
                      <Card key={pile.id} className="p-4">
                        <div className="flex gap-4">
                          <div className="w-20 h-28 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 flex items-center justify-center">
                            {pile.imagePath ? (
                              <img 
                                src={getImageUrl(pile.imagePath)} 
                                alt={pile.name}
                                className="w-full h-full object-cover"
                                onError={(e) => { 
            ...
            {adminTab === 'prices' && (
              <div>
                ...
                <Card className="p-6 mb-6">
                  ...
                  <div className="flex gap-2 mt-4">
                    <Button onClick={handleSavePrice} className="h-9 px-4 text-sm font-medium bg-red-600 hover:bg-red-700 text-white">{editingPrice ? 'Update' : 'Create'}</Button>
                    <Button onClick={() => { setShowPriceForm(false); setEditingPrice(null) }} variant="outline" className="h-9 px-4 text-sm font-medium">Cancel</Button>
                  </div>
                </Card>
                )}
                      ...
                      <Card key={price.id} className="p-4">
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex gap-1">
                            ...
                            <Button onClick={() => { if(confirm('Delete this price?')) handleDeletePrice(price.id) }} size="sm" variant="ghost" className="h-8 w-8 p-0 text-red-600 hover:text-red-700"><Trash2 className="w-4 h-4" /></Button>
                          </div>
                        </div>
                      </Card>
                    )
